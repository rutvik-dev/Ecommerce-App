...
let SignupUser = async function (req, res, next) {
    try {
        ...
        const CreateUser = await User.create({ username, email, password: hashedPassword , OrderId });
...
let ReadAllUser = async function (req, res, next) {
    try {
        ...
        const ReadUsers = await User.find().populate('OrderId');
        res.status(201).json({
            status: "success",
...
let ReadSingleUser = async function (req, res, next) {
    try {
        ...
        const user = await User.findById(id).populate('OrderId');
        if (!user) {
...
let UpdateUser = async function (req, res, next) {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(id, req.body, { new: true }).populate('OrderId');
        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }
        res.status(201).json({
